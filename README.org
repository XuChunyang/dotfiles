...
** TODO Safer tangle by preventing overriding or showing diff before overriding?
   ...
   Try the following command instead of ~org-babel-tangle~. Notes
   - Not work with ~/.authinfo.gpg because GPG simply generates
     different binary for the same input every time.
   - Not work with remote files
...
** Hooks
   ...
   #+BEGIN_SRC sh :tangle ~/Maildir/.notmuch/hooks/post-new :mkdirp yes :shebang "#!/usr/bin/env bash" :commants link
